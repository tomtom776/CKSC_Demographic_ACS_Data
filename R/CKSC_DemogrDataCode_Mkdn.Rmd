---
title: "CKSC_DemogrDataCode_Mkdn"
author: "Thomas Quintero"
date: "1/8/2021"
description: To create a Shapefile of ACS data by Census Tract or Zip Code.
...
## CKSC Demographic Parsing Code

This R Markdown document features the code used to parse ACS demographic data which is used in an ArcGIS Online Web App <https://tq.maps.arcgis.com/apps/webappviewer/index.html?id=6847dbf1f3304d5280b980e39721ab3e> for grant applications. 

```{r Libraries & FIPS Assignment}
#************************#
# Libraries
#************************#
library(tigris)
...
# Choose your working directory (where you want the Shapefile saved). Click 'Session', 'Set Working Directory', then 'Choose Directory...' OR just do Ctrl + Shift + H

DATA_YEAR = 2019
DATA_SOURCE = 'ACS'
ACS_YEAR_SPAN = '5'
GEOGRAPHY = 'Tracts'  #Type 'Tracts' verbatim with capital 'T' or 'ZipCode'
COUNTIES = c(Wayne, Macomb, Oakland) # Choose from the above counties which you desire
```

```{r API Key, Boundary, and File Name}
#************************#
# API Key
#************************#
...
savename <- paste("MI", DATA_YEAR, DATA_SOURCE, ACS_YEAR_SPAN, GEOGRAPHY, "DemogData.shp", sep = "_")

#************************#
# Geographies, Polygons
#************************#
#zip code filtered to those starting with 48 and 49
zipcode <- zctas(state = 'MI', starts_with = c('48','49'), cb = TRUE, year = DATA_YEAR)
#tracts filtered to counties
tracts <- tracts(state = 'MI', county = COUNTIES, cb = TRUE, year = DATA_YEAR)

#************************#
# Geographies, ACS Query
#************************#
ACS_zip <- geo.make(zip.code = '*')
ACS_tracts <- geo.make(state = "MI", county = COUNTIES, tract="*")

#************************#
# Setting Boundaries for Data
#************************#
if (GEOGRAPHY == "Tracts"){
  BOUNDARY = tracts
  ACS_BOUNDARY = ACS_tracts
} else {
  BOUNDARY = zipcode
  ACS_BOUNDARY = ACS_zip
  names(BOUNDARY)[3] <- "GEOID"
}
```

```{r Demographic Parsing and Export to Shapefile}
#************************#
# ----Age and Sex % B01001----
#************************#
age <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B01001", col.names = "pretty")
...
# ----Race B02001----
#************************#
race <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B02001", col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
#************************#
hisp <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, variable = acs.lookup(endyear=2014, span=5, table.number="B03001"), col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
# ----Nativity B05002----
#************************#
nativity <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B05002", col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
#************************#
house <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B11001", col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
#************************#
ed <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B15002", col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
#************************#
pov <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B17001", col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
#************************#
disab <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B18101", col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
#************************#
income <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B19013", col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
#************************#
vet <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B21001", col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
#************************#
employ <- acs.fetch(endyear = DATA_YEAR, span = ACS_YEAR_SPAN, geography = ACS_BOUNDARY, table.number = "B23001", col.names = "pretty")
# use of col.names = "pretty" above gives the full column definitions
# if you want Census variable IDs use col.names="auto".
...
merged <- geo_join(BOUNDARY, merge_me, "GEOID", "GEOID")
EXPORT <- merged

#************************#
# ----Exporting to Shapefile----
#************************#
st_write(EXPORT, savename, driver = "ESRI Shapefile")
```
